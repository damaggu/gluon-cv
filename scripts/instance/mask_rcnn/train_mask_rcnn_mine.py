...
def load_mask(img, train_segm):
   ...
   Returns:
    ...
    mask = np.zeros([img.shape[0], img.shape[1], len(train_segm)], dtype=np.uint8)
    for i, p in enumerate(train_segm):
...
def get_dataset(dataset, args):
    if dataset.lower() == 'blbalbla':
        ...
        def jsonToSet(annotations_train_1,
                     img_path):
            for key in tqdm(annotations_train_1.keys()):
                el = annotations_train_1[key]

                img = mx.nd.array(imread(img_path + key.split('.')[0]+'.png').astype('float32'))
                ...
                for reg in el['regions']:
                    ...
                    _mask = [np.swapaxes(np.asarray([x,y]).astype('float32'), axis1=0, axis2=1)]

                    masks.append(_mask)

        #             print(masks[-1])
                    mask = load_mask(img.asnumpy(), np.expand_dims(_mask,axis=-1))
        #             print(mask[0].shape)
                    bboxes = extract_bboxes(mask)
        #             print(bboxes)
                    bboxes = [[bboxes[0][1],bboxes[0][0],bboxes[0][3],bboxes[0][2]]]
        #             print(bboxes)
...
def get_dataloader(net, train_dataset, val_dataset, train_transform, val_transform, batch_size,
    ...
    train_sampler = \
        gcv.nn.sampler.SplitSortedBucketSampler(list(np.ones((len(train_dataset),))),
                                                batch_size,
